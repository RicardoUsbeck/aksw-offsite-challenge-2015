...
public class RMSD {
     ...
     * 
     * @param response
     *            Output of an algorithm, read from a file.
     * @param rankRange
     *            Rank ranges computed by executing real queries. Map a resource
     *            to a list with allowed begin and end rank
    ...
    public static double getRMSD(List<String> response, Map<String, List<Double>> rankRange) {
        ...
        for (int i = 0; i < response.size(); i++) {
            ...
            if (rankRange.containsKey(resource)) {
                ...
                // if resource not within acceptable range then increase total
                // error
                if (!(i >= range.get(0) && i <= range.get(1))) {
                    error = error + Math.min(Math.pow(i - range.get(0), 2), Math.pow(i - range.get(1), 2));
                }
            }
        }

        // now check for foundResources that were not ranked and assign them the
        // max error
